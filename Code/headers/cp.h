	...
	int cp_nf_tarf(char *src,char *dest){
			...
			if(diff(split(dest,1),path_behind1(dest))!=1 && occurences1(path_behind1(dest))==0){
	...
	int cp_ftar_ftar(char *src,char *dest){
		...
		if(cmp(split(dest,2),file_name(dest))!=0 && occurences1(path_behind1(dest))==0){
	...
	int cp_ftar_tardir(char *src ,char *dest){
		...
		strcat(name,file_name(src));	
		if(cmp(split(src,2),"")!=0 && diff(split(src,1),path_behind1(src))!=1 && occurences1(path_behind1(src))==0){
			write(1,"cp : source directory : no such file or directory\n",strlen("cp : source directory : no such file or directory\n"));
			return -1;
			}
		...
		while(read(s,&p2,512)>0 && p2.name[0]!='\0'){
			...
			if(cmp(split(src,2),p2.name)==0){
				...
				read(s,&p2,512);			
					...
					memset(&p2.junk, '\0', 12);
					char *n=strdup("");
					if(diff(split(dest,1),path_behind1(dest))!=1){	
						strcat(n,dest);
						strcat(n,file_name(src));
						if(occurences1(path_behind1(n))>0){
	...
	int copy_tars(char *src ,char *dest){
		...
		while(read(s,&p1,512)>0 &&  p1.name[0]!='\0'){
			...
			if(p1.typeflag=='5'){
				...
				cp_ftar_tardir(names,path_behind1(named));
				i=0;
	...
	int cp_tardir_ndir(char *src ,char *dest){
		 ...
		 if(dirp==NULL && cmp(path_behind1(dest),"")==0){
		 ...
		 else if(dirp==NULL && opendir(path_behind1(dest))==NULL){
			 write(1,"cp -r : destination directory : can not create directory\n",strlen("cp -r : destination directory : can not create directory\\n"));
			return -1;
			}
		lseek(s,0,SEEK_SET);
			...
			if(cmp(split(src,2),p.name)==0){
				...
				if(cmp(p.name,split(src,2))!=0 && diff(split(src,2),p.name)==-1){
					if(p.typeflag!='5'){
							...
							strcat(n,p.name);
							char *d=strdup("");
							if(cmp(path_behind1(dest),"")!=0)
								strcat(d,path_behind1(dest));
							else
								strcat(d,dest);
							if(dest[strlen(dest)-1]!='/')
								strcat(d,"/");
